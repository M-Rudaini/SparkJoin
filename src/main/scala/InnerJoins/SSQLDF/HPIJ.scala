...
object HPIJ {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Hash Repartitioning...
    val PUserDF = InUserDF.repartition(200, $"User_ID")

    //Hash Repartitioning...
    val PPurchaseDF = InPurchseDF.repartition(200, $"User_ID")

    //Selecting the Required Columns from the Users DataFrame...
    //Concatenate first and last names as new column "Full_Name"...
    val UserDF = PUserDF.select($"User_ID", concat($"First_Name", lit(" "), $"Last_Name")as "Full_Name")
    ...
    val PDF1 = PPurchaseDF.select($"User_ID", $"Unit_Amount" * $"Unit_Price" as "Total_Price")
