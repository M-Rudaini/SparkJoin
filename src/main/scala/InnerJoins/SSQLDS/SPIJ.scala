...
object SPIJ {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val UDS1 = InUserDS.select($"User_ID", concat($"First_Name", lit(" "), $"Last_name") as "Full_Name")
    UDS1.show(1)
    UDS1.printSchema()

    //-Select the Wanted Columns From the Purchases Dataset
    //--Multiplying unit amount and unit price as "Total_Price"...
    val PDS1 = InPurchseDS.select($"User_ID", $"Unit_Amount" * $"Unit_Price" as "Total_Price")
    PDS1.show(1)
    PDS1.printSchema()
    ...
    val UPDS1 = UDS1.join(PDS1,"User_ID")
    UPDS1.show(10)
    UPDS1.printSchema()
    ...
    val UPDS2 = UPDS1.groupBy($"User_ID",$"Full_Name")

    //-Aggrigating through the grouped values summing all of them as "Total_Purchase".
    val UserPurchase =UPDS2.agg(sum($"Total_Price") as "Total_Purchase")
